...
public class MainFrame extends JFrame{
    ...
    private JButton EXAMButton; //ON LINE 172
    ...
    public MainFrame() {
        ...
        EXAMButton.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                if(btnPriority1.isSelected()){
                    for (int i = 0; i < port.hubs.length; i++) {
                        int x=port.hubs[i].method1(1);
                        textFieldExam.setText(Integer.toString(x));
                        // int x= port.method2(1);
                    }
                }
                else if(btnPriority2.isSelected()){
                    for (int i = 0; i < port.hubs.length; i++) {
                        int x=port.hubs[i].method1(2);
                        textFieldExam.setText(Integer.toString(x));
                    }
                   //int x= port.method2(2);
                   // textFieldExam.setText(Integer.toString(x));

                }
                else if(btnPriority3.isSelected()){
                    for (int i = 0; i < port.hubs.length; i++) {
                        int x=port.hubs[i].method1(3);
                        textFieldExam.setText(Integer.toString(x));
                    }
                  // int x= port.method2(3);
                  //  textFieldExam.setText(Integer.toString(x));
                }
            }
        });
